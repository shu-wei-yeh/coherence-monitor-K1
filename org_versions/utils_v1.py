...
def get_observing_segs(t1, t2, ifo, min_duration=3600):
    tstart = to_gps(t1)
    tend = to_gps(t2)
    if ifo != 'K1':
        raise ValueError(f"Unsupported interferometer: {ifo}. Must be 'K1'.")
    try:
        segs = DataQualityFlag.query(f'{ifo}:GRD_SCIENCE_MODE:1', t1, t2)
    except Exception as e:
        raise RuntimeError(f"Failed to query data quality flags for {ifo}: {e}")
    seg_list = SegmentList()
    for segment in segs.active:
        if segment.end - segment.start > min_duration:
            seg_list.append(segment)
    print(f"Retrieved {len(seg_list)} segments for {ifo} with minimum duration {min_duration}s.")
    return seg_list
...
def get_max_corr_band(output_dir, flow = 55, fhigh = 65, save=False):
    """
    Process all CSV files in `output_dir` to compute the maximum correlation value
...
#             base_name = os.path.basename(file_path).split("/")[-1].split("_14")[0]
...
def plot_max_corr_chan(path, fft, ifo, duration, flow=0, fhigh=400):
    """
